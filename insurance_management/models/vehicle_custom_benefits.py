...
class vehicle_custom_benefit(models.Model):
    _name = 'vehicle.custom.benefit'
    _inherit = ['mail.thread', 'mail.activity.mixin', 'mail.render.mixin']
    _description = 'vehicle_custom_benefit'

    name = fields.Char()
    benefit_name = fields.Char(store=True)
    insurance_company_id = fields.Many2one(related='vehicle_quotation_id.insurance_company_id',string='Insurance Company')
    category_type = fields.Selection([('vip','VIP'),('a+','A+'),('a','A'),('b','B'),('c','C')],string='Category Type')
    benefit_id = fields.Many2one('benefit.name',string='Benefit',domain="[('ins_type_select','=','is_vehicle')]")
